...
// single baseline it always was; one too wide used to overflow the border and
// run into the neighbouring hint, and now wraps to at most two centred lines
...
void BaseTheme::drawHintLabel(GfxRenderer& renderer, const int fontId, const char* label, const int x,
  ...
  // button and clips the second one. The wrapped block sits centred in the
  // button; the bottom of a second line is still slightly clipped, since the
  // visible area inside the button is shorter than boxHeight — legible enough,
  // and tightening the gap or top-aligning made no visible difference on device.
